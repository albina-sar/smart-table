...
export function initData(sourceData) {
    ...
    const filterRecords = (records, query) => {
        return records.filter(record => {
            ...
            // Фильтр по сумме (от)
            if (query['filter[totalFrom]'] && record.total_amount < Number(query['filter[totalFrom]'])) {
                return false;
            }
            
            // Фильтр по сумме (до)
            if (query['filter[totalTo]'] && record.total_amount > Number(query['filter[totalTo]'])) {
                return false;
            }
    ...
    const getIndexes = async () => {
        if (!sellers || !customers) {
            ...
            sellers = createIndex(sourceData.sellers, 'id', v => `${v.first_name} ${v.last_name}`);
            customers = createIndex(sourceData.customers, 'id', v => `${v.first_name} ${v.last_name}`);
        }
